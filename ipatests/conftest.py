...
MARKERS = [
    ...
    ('skip_if_hostplatform(host attribute name within IntegrationTest, index '
     'of host within hosts list if required, platform name, reason): Skip '
     'integration test on remote platform (ID and ID_LIKE)'),
    ('skip_if_hostcontainer(host attribute name within IntegrationTest, index '
     'of host within hosts list if required, container type, reason): Skip '
     'integration test on remote container ("any" or specific type)'),
    ('skip_if_hostfips(host attribute name within IntegrationTest, index '
     'of host within hosts list if required, reason): Skip integration test on'
     ' remote host in FIPS mode'),
    ('skip_if_not_hostselinux(host attribute name within IntegrationTest, '
     'index of host within hosts list if required, reason): Skip integration '
     'test on remote host not in SELinux mode'),
    ('skip_if_host(host attribute name within IntegrationTest, index '
     'of host within hosts list if required, condition callback, reason): Skip'
     ' integration test on remote host based on condition callback result'),
]
...
def process_hostmarker(pytest_mark, pytest_nodeid, pytest_cls):
    ...
    hosts = getattr(pytest_cls, hostattr)
    ...
    if pytest_mark.name == "skip_if_hostplatform":
        ...
        if platform in host.osinfo.platform_ids:
            pytest.skip(
                f"{pytest_nodeid}: Skip test on remote host "
